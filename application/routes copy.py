...
import tempfile
from application.hand_detection import detect_sign_language
#omr
...
def sign_language():
    # Xoá dữ liệu khi trang tải lại
    session.pop("sentence", None)
    return render_template("sign_language.html", title="Sign Language")

@app.route('/save_detected_text', methods=['POST'])
def save_detected_text():
    detected_text = request.json.get("text", "").strip()  # Đảm bảo strip()
    session["sentence"] = detected_text
    return jsonify({"status": "success", "message": "Detected text saved to session."})
